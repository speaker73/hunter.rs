...
impl MainState {
    fn new() -> MainState {
        MainState { pos_x: 100.0, pos_y: 100.0 }
    }
}
...
fn render_row(end:i32, ctx: &mut Context, iter:i32, min_width:i32, rows:i32, invert:bool){
    ...
    for i in 0..end{
        ...
        let padding_left = 2;
        let padding_top = -2;         
        let mut x = 310 - ((end-min_width) * 4) + (i * (w + padding_left) ) - ((end-min_width) * (w/2) );
        ...
        graphics::polygon(ctx,DrawMode::Fill,
                       &[Point { x: (x - w/2) as f32, y: (y - h/2)as f32  },
                         Point { x: x as f32, y: (y - h) as f32 },
                         Point { x: (x + w/2) as f32, y: (y - h/2) as f32 },
                         Point { x: (x - w/2) as f32, y: (y - h/2) as f32  }]).unwrap();
        ...
        //graphics::circle(ctx, DrawMode::Fill, Point { x: (x -30) as f32, y: (y - 25)as f32 }, 2.0, 100).unwrap(); 
    }              
}
...
impl event::EventHandler for MainState {
   ...
   /* fn mouse_button_down_event(&mut self, button: MouseButton, x: i32, y: i32) {
        println!("Mouse button pressed: {:?}, x: {}, y: {}, pos_x: {}", button, x, y, self.pos_x);
        self.pos_x = x as f32;
        self.pos_y = y as f32;        
    }
